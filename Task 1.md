...
- 7.49：对于 `combine` 函数的三种不同声明，当我们调用 `i.combine(s)` 时分别发生什么情况？其中 `i` 是一个 `Sales_data` ，而s 是一个 `string` 对象。

  **(a)** `Sales_data &combine(Sales_data);`

  > 正确。编译器隐式地调用 `Sales_data` 的构造函数，生成一个临时的 `Sales_data` 对象，然后传递给 `combine` 的形参。

  **(b)** `Sales_data &combine(Sales_data&);`

  > 错误。编译无法通过。因为 `combine` 成员函数的形参是非常量引用，但是s自动创建的 `Sales_data` 临时对象无法传递给 `combine` 所需的非常量引用。

  **(c)** `Sales_data &combine(const Sales_data&) const;`

  > 错误。编译无法通过。因为我们把 `combine` 成员函数声明成了常量成员函数，所以该函数无法修改数据成员的值。
